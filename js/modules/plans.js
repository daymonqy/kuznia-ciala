  ...
  showAddExerciseModal(planId, dayIndex, existing = null, editIndex = null) {
    ...
    Utils.showModal(`
      ...
      <div class="form-group">
        <label>1. Wybierz ćwiczenie z listy</label>
        <select class="form-select" id="pex-select" style="font-size:15px;padding:14px 12px">
          <option value="">— kliknij i wybierz ćwiczenie —</option>
        </select>
      </div>
      <div class="form-group">
        <label>Szukaj (opcjonalnie)</label>
      ...
      <div style="position:sticky;bottom:0;background:var(--bg-elevated);padding-top:12px;border-top:1px solid var(--border);margin-top:16px;z-index:5">
        <button type="button" class="btn btn-primary btn-block" id="pex-confirm" style="font-size:16px;padding:16px">
          ${isEdit ? 'Zapisz zmiany' : '✓ Dodaj do planu'}
        </button>
    ...
    const fillSelect = () => {
      ...
      sel.innerHTML = '<option value="">— kliknij i wybierz (' + list.length + ') —</option>' +
        list.map(ex => {
          let badge = '';
      ...
      if (cnt) cnt.textContent = list.length ? `Dostępnych: ${list.length}` : 'Brak wyników — zmień filtr';
    };
    ...
    document.getElementById('pex-confirm')?.addEventListener('click', (e) => {
      ...
      try {
        ...
        if (!exerciseId) {
          Utils.toast('Najpierw wybierz ćwiczenie z listy');
          document.getElementById('pex-select')?.focus();
          return;
        }
        syncRowsFromDom();
        if (!setRows.length) {
